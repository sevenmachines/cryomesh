...
CryoManager::CryoManager() :
		currentState(EMPTY), creator(), database() {
	srand(time(NULL));time_t rawtime;
	time(&rawtime);
...
void CryoManager::storeCurrentActualOutputPatterns(
	...
	// forall in current_patterns
	{
		...
		const std::map<boost::uuids::uuid, boost::shared_ptr<state::Pattern> >::const_iterator it_current_patterns_end =
				current_patterns.end();
